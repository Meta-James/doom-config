...
`emacs-rebuild`, `firefox-chrome-install`, `emacs-magnet`, `desktop-install`, or
the `~/.local/share/applications/` entries `org-protocol.desktop`,
`emacs-magnet.desktop` and `emacs-mailto.desktop`) is overwritten by the next tangle, which happens on
