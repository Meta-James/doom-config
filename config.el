...
;; gives transcript fetching (`F' in the search and show buffers) -- that half
;; works. The mpv follow-along half does not, and not for the reason this
;; comment used to give: `mpv' and `yt-dlp' are both installed now, but
;; `elfeed-tube-mpv-follow-mode' and `elfeed-tube-mpv-where' live in
;; elfeed-tube-mpv.el, a separate MELPA package depending on the `mpv' package,
;; and Doom's `:app rss +youtube' declares neither. The module still binds
;; `C-c C-f' and `C-c C-w' to them in `elfeed-show-mode-map', so both keys are
;; void-function until (package! mpv) and (package! elfeed-tube-mpv) are added.
;; See docs/decisions.org for the RSS integration ADR.
